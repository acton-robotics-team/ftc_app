...
class Hardware(hwMap: HardwareMap, private val opMode: LinearOpMode) {
     ...
     */
    fun turn(deg: Float) {
        var rad = deg * Math.PI / 180 + lastTargetedPosition * Math.PI /180
        log("turning to" + rad)
    ...
    fun turnFromStart(deg: Float) {
        ...
        setDrivePower(0.0)
        log("finished turning")
        lastTargetedPosition = (rad * 180 / Math.PI).toFloat()
    }
    ...
    companion object {
        ...
        const val LIFTER_TOP_POSITION = 16000
